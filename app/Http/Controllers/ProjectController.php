...
class ProjectController extends Controller
{
    ...
    public function storeProject(Request $request)
    {
        try {
            ...
            // Cek apakah jumlah proyek untuk service yang dipilih sudah mencapai 5
            $service = Service::findOrFail($validated['project_service']);
            if ($service->projects()->count() >= 5) {
                return redirect()->route('manage-project')->with('error', 'This service already has 5 projects. Cannot add more projects.');
            }
    ...
    public function updateProject(Request $request, $id)
    {
        try {
            ...
            $project->update([
                ...
                'project_description' => $validated['project_description'],
                'project_service' => $validated['project_service'],
                'project_link' => $validated['project_link'] ?? $project->project_link, // Hanya update link jika ada
            ]);
