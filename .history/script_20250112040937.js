...
let swiperPortfolio = new Swiper(".services-box-services", {
  ...
  slidesPerView: "auto", // Allow dynamic resizing of slides
  navigation: {
  ...
  on: {
    init: function () {
      if (this.slides.length <= 1) {
        this.loop = false; // Disable loop if only one slide
        this.slides[0].style.opacity = 1; // Ensure the first slide is visible
    ...
    slideChange: function () {
      ...
      this.slides[this.activeIndex].style.opacity = 1; // Show active slide
    },
  },
});
...
function updateSwiperForMobile() {
  ...
  if (isMobile) {
    swiperPortfolio.params.slidesPerView = 1; // Set to 1 slide for mobile
    swiperPortfolio.params.spaceBetween = 20; // Adjust spacing
  } else {
    swiperPortfolio.params.slidesPerView = "auto"; // Restore default for desktop
    swiperPortfolio.params.spaceBetween = 10; // Restore default spacing
  }
  swiperPortfolio.update(); // Update swiper configuration
}
