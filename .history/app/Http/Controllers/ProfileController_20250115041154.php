...
class ProfileController extends Controller
{
    ...
    public function updateProfile(Request $request, $id)  // Capture the user ID from the route
    {
        // Log the incoming request
        \Log::info('Profile update request:', $request->all());

        // Get the user by ID (use Auth::user() to ensure they are updating their own profile)
        $user = User::findOrFail($id);

        // Make sure the authenticated user is the one updating their own profile
        if ($user->id !== Auth::id()) {
            return redirect()->route('profile.page')->with('error', 'You are not authorized to update this profile.');
        }
