                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <form id="delete-form-{{ $project->id }}" action="{{ route('delete-project', $project->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            
                            <!-- Tombol Edit, membuka modal edit dengan data proyek -->
                            <button class="btn btn-warning btn-sm" 
                                data-bs-toggle="modal" 
                                data-bs-target="#editProjectModal" 
                                onclick="openEditModal({{ $project->id }}, '{{ $project->project_name }}', '{{ $project->project_description }}', {{ $project->project_service }}, '{{ $project->project_link }}')">
                                Edit
                            </button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editProjectForm" action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label for="projectDescription" class="form-label">Project Description</label>
                <textarea class="form-control" id="projectDescription" name="project_description" rows="3" required></textarea>
            </div>
            <!-- Service Name Dropdown -->
            <div class="mb-3">
                <label for="serviceName" class="form-label">Service Name</label>
            ...
            <div class="mb-3">
                <label for="projectPhoto" class="form-label">Project Photo</label>
                <input type="file" class="form-control" id="projectPhoto" name="project_photo" accept="image/*">
                <div class="form-text">Leave empty if you don't want to change the photo.</div>
            </div>
            <!-- Project Link -->
            <div class="mb-3">
                <label for="projectLink" class="form-label">Project Link</label>
                <input type="url" class="form-control" id="projectLink" name="project_link">
            </div>
            <!-- Submit Button -->
            <div class="text-end">
...
<script>
     function openEditModal(projectId, projectName, projectDescription, projectService, projectLink) {
        // Isi data ke dalam form modal
        document.getElementById('projectName').value = projectName || ''; // Jika kosong, isi dengan string kosong
        document.getElementById('projectDescription').value = projectDescription || '';
        document.getElementById('serviceName').value = projectService || ''; // Pastikan projectService adalah ID yang valid
        document.getElementById('projectLink').value = projectLink || '';

        // Update form action untuk mengirimkan id proyek yang benar
        var form = document.getElementById('editProjectForm');
        form.action = '/manage-project/update/' + projectId; // Sesuaikan URL dengan route yang benar
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (form) {
                    ...
                    console.error('Delete form not found for project ID:', projectId);
                }
            }
        });
    }
